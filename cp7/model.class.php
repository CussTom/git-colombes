...
/**
 * Mini framework en PHP orienté objet qui permet de réaliser un CRUD sur une table d'une BDD Mysql ou MariaDB
 * Etand la classe Singleton
 */
...
class Model extends Singleton
{
    ...
    public function __construct(string $newHost, int $newPort, string $newDbName, string $newUser, string $newPass, string $newTable, array $newOptions = array())
    {
     ...
     * @return array resultat de la requête SELECT
     */
    ...
    /**
     * Méthode qui insert une ligne dans la table
    ...
    public function insert(array $post = array()): int
    {
        if (empty($post)) {
            ...
            // Remplit tableau des paramètres
     ...
     * @param string $id - valeur de l'ID recherché
     * @return int nombre de ligne impactées (défaut 1)
     */
     ...
     * @return int nombre de ligne supprimée (défaut 1)
     */
